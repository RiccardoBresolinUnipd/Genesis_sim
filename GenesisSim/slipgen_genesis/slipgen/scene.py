...
def setup_camera(scene):
    ...
    # 1. Camera Esterna (Fissa)
    cam_ext = scene.add_camera(
        res    = (224, 224),
        pos    = (1.2, 1.2, 1.2),    # Più lontana per vedere tutto il tavolo
        lookat = (0.4, 0.0, 0.2),
        fov    = 45,
    )
    ...
    cam_wrist = scene.add_camera(
        res    = (640, 480),
        #attach_to = end_eff,         # Si muove con la mano!
        pos    = (0.1, 0.0, 0.05),   # Posizionata leggermente sopra le dita
        lookat = (0.2, 0.0, 0.0),    # Guarda verso il centro della pinza
        fov    = 75,                 # FOV più ampio per non perdere l'oggetto da vicino
    )
    return cam, cam_ext, cam_wrist
...
def init_scene(show_viewer: bool = True, cube_area=None, num_cubes: int = 3) -> Tuple:
    ...
    cam, cam_ext, cam_wrist = setup_camera(scene)
    ...
    return scene, franka, cam, cam_ext, cam_wrist, end_effector, cubes, motors_dof, fingers_dof
...
def setup_with_knobs(knobs: SlipKnobs, show_viewer: bool = True, cube_area=None, num_cubes: int = 1) -> Tuple:
    """Legacy combined setup (creates scene + applies knobs in one call)."""
    scene, franka, cam, cam_ext, cam_wrist, end_effector, cubes, motors_dof, fingers_dof = init_scene(show_viewer=show_viewer, cube_area=cube_area, num_cubes=num_cubes)
